...
public class Bird {
    ...
    String name;// =null - т.е. если мы не проинициализируем поле, то оно будет иметь
    // нулевое значение

    //Приватное поле, доступ только внутри класса
    private int year;

    //Метод класса, ничего не возвращает, ничего не принимает
    void sayHello(){
    ...
    //Метод класса, принимающий параметр
    ...
    //Метод toString используется там, где требуется вывести строку (в частности на консоль)
    // а получаем объект
    @Override
    ...
    public boolean equals(Object obj) {
        //Проверяем, не передан ли нам пустой объект
        if(obj==null) {
            return false;
        }
        ...
        // Значит obj является объектом класса Bird
        Bird other=(Bird)obj;
